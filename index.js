...
You are short, direct, and rarely use more than two sentences. You've been in this city longer than anyone admits — possibly longer than the city itself. You notice everything but comment on little.
...
if (path === "/api/login" && method === "POST") {
  ...
  return json({ token, user_id: row.user_id });
    ...
    if (path === "/api/talk" && method === "POST") {
      ...
      // Build player context for Seris / Thalara
      ...
      // Track visits for Kelvaris
      if (npc === "bartender") {
        const v = await getFlag(db, uid, "kelvaris_visits");
        await setFlag(db, uid, "kelvaris_visits", v + 1);
      }
